...
void Delaunay::initialize(const glm::vec4& boundingBox) {
 ...
 * @y: 新点y坐标
 */
void Delaunay::addPoint(const glm::vec2 & newPoint) {
	// 记录空腔
...
void Delaunay::deleteCommonEdges(std::list<Edge>& polygon) {
	// 查找三角形公共边
...
void Delaunay::deleteSuperTriangle() {
	for (std::list<Triangle>::iterator it = triangleVector.begin(); it != triangleVector.end(); ) {
